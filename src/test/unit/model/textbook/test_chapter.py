...
def test_new_success_creates_chapter(chapter_title):
    """新規作成時にIDとタイトルが設定され、本文は空文字になること。"""
    # Act
    chapter = Chapter.new(title=chapter_title)
...
def test_new_failure_invalid_title_type(title):
    """TitleString以外のタイトルでは作成できないこと。"""
    # Assert
    with pytest.raises(ValueError):
        Chapter.new(title=title)


def test_set_title_success_updates_title(chapter_title):
    """タイトルを更新できること。"""
    # Arrange
    chapter = Chapter.new(title=chapter_title)
...
def test_set_title_failure_invalid_title_type(chapter_title, title):
...
def test_set_content_success_accepts_empty_content(chapter_title):
    """本文を空文字に更新できること。"""
    # Arrange
    chapter = Chapter.new(title=chapter_title)
...
def test_set_content_failure_invalid_content_type(chapter_title, content):
    """文字列以外の本文では更新できないこと。"""
    # Arrange
    chapter = Chapter.new(title=chapter_title)
